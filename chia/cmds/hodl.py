...
def programs_cmd(fingerprint: int, json: bool, verbose: int) -> None:
    from chia.hodl.cli.cmd_programs import cmd_programs
    from chia.hodl.util import callCliCmdHandler
    asyncio.run(
        callCliCmdHandler(
            cmd_programs,
            verbose,
...
@click.option("-P", "--program",
              help="HODL program name to commit to. See `chia hodl programs` for options.",
...
def commit_cmd(program: str,
    ...
    from chia.hodl.cli.cmd_commit import cmd_commit
    from chia.hodl.util import callCliCmdHandler
    asyncio.run(
        callCliCmdHandler(
            cmd_commit,
...
def cancel_cmd(contract: str,
    ...
    from chia.hodl.cli.cmd_cancel import cmd_cancel
    from chia.hodl.util import callCliCmdHandler
    asyncio.run(
        callCliCmdHandler(
            cmd_cancel,
...
def limits_cmd(fingerprint: int,
    ...
    from chia.hodl.cli.cmd_limits import cmd_limits
    from chia.hodl.util import callCliCmdHandler
    asyncio.run(
        callCliCmdHandler(
            cmd_limits,
...
def show_cmd(contract: str,
    ...
    from chia.hodl.cli.cmd_show import cmd_show
    from chia.hodl.util import callCliCmdHandler
    asyncio.run(
        callCliCmdHandler(
            cmd_show,
...
def profits_cmd(fingerprint: int,
    ...
    from chia.hodl.cli.cmd_profits import cmd_profits
    from chia.hodl.util import callCliCmdHandler
    asyncio.run(
        callCliCmdHandler(
            cmd_profits,
