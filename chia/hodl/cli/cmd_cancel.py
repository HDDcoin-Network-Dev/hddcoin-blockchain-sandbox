...
from chia.hodl.val import validateCancellation
...
async def cmd_cancel(hodlRpcClient: HodlRpcClient,
    ...
    mempool_tx = await chia.hodl.util.cancelContract(hodlRpcClient,
                                                        fullNodeRpcClient,
                                                        contractDetails)
    print(
        f"\n"
        ...
        f"   ==> Monitor contract status directly with `{Y}chia hodl show -C {contract_id}{_}`\n"
        f"   ==> The mempool tx_id for the request is {Y}{mempool_tx}{_}\n"
        f"   ==> Watch your wallet balance for progress with `{Y}chia wallet show{_}`\n"
    )
