# -*- coding: utf-8 -*-
# NOTES:
#  - this file contains the CLI implementation for `chia hodl commit`
...
import chia.hodl
import chia.hodl.exc as exc
import chia.util
from chia.hodl.hodlrpc import HodlRpcClient
from chia.hodl.util import vlog
from chia.hodl.val import validateContract
...
async def cmd_commit(hodlRpcClient: chia.hodl.hodlrpc.HodlRpcClient,
                     *,
                     config: th.Dict[str, th.Any],
                     walletRpcClient: chia.rpc.wallet_rpc_client.WalletRpcClient,
                     program_name: str,
    ...
    fee_bytes = chia.util.ints.uint64(int(fee_hdd * chia.hodl.BYTES_PER_HDD))
    ...
    if not payout_address:
        ...
        payout_address = chia.hodl.util.getFirstWalletAddr(config, hodlRpcClient.sk)
        vlog(1, f"payout_address automatically set to {payout_address}")

    vlog(1, f"Fetching contract from HODL server")
    deposit_bytes = chia.util.ints.uint64(int(commit_hdd * chia.hodl.BYTES_PER_HDD))
...
async def _cli_checkWallet(config: th.Dict[str, th.Any],
    ...
    sys.stdout.flush()
    try:
        await chia.hodl.util.verifyWalletFunds(walletRpcClient, wallet_id, requiredFunds_hdd)
        ...
        print(f"  ==> Check your spendable wallet balance with `{Y}chia wallet show{_}`")
    except exc.WalletTooFragmented as e:
        maxSend_bytes = int(e.args[0])
        maxSend_hdd = decimal.Decimal(maxSend_bytes) / chia.hodl.BYTES_PER_HDD

        fp = hodlRpcClient._fingerprint
        defragCmd = f"chia wallet defrag -f {fp}"
...
def _confirmContractRequest(fingerprint: int,
    ...
    prompt = (
        ...
        f" 4. Your HODL'd funds will be clearly visible in `{Y}chia wallet show{_}` output\n"
        f" 5. You will get a receipt describing the contract, and how to manage it\n"
        f" 6. You can always cancel the contract later (with `{Y}chia hodl cancel{_}`)\n"
        f"\n"
        ...
        f" - Check current HODL contract limits with `{Y}chia hodl limits{_}`\n"
        f"\n"
    ...
    print(prompt, end = "")
    response = input("")
    if (time.monotonic() - start_s) > chia.hodl.CONTRACT_EXPIRATION_TIME_s:
...
def _stashPrecommitReceipt(receipt: th.Dict[str, th.Any]) -> pathlib.Path:
    chia.hodl.HODL_DIR_TMP.mkdir(parents = True, exist_ok = True)
    contract_id = receipt["receipt_info"]["contract_id"]
    precommitPath = chia.hodl.HODL_DIR_TMP / f"{contract_id}.yaml"
    with open(precommitPath, "w") as fp:
        fp.write(yaml.dump(receipt, sort_keys = False))
    return precommitPath


async def _createContractCoin(walletClient: chia.rpc.wallet_rpc_client.WalletRpcClient,
                              fingerprint: int,
                              wallet_id: int,
                              deposit_bytes: chia.util.ints.uint64,
                              fee_bytes: chia.util.ints.uint64,
                              contract_address: str,
                              contract_id: str,
                              ) -> th.Tuple[str, bool]:
    tx: chia.wallet.transaction_record.TransactionRecord

    deposit_hdd = decimal.Decimal(deposit_bytes) / chia.hodl.BYTES_PER_HDD
...
def _storeFinalReceipt(precommitReceiptPath: pathlib.Path) -> pathlib.Path:
    chia.hodl.HODL_DIR_RECEIPTS.mkdir(parents = True, exist_ok = True)
    finalReceiptPath = chia.hodl.HODL_DIR_RECEIPTS / precommitReceiptPath.name
...
def _printFinalSummary(finalReceiptPath: pathlib.Path,
    ...
    p(f" ==> Monitor contract status with `{Y}chia hodl show -C {contract_id}{_}`")
    ...
    p(f"    `{Y}chia wallet get_transaction -f {fingerprint} -tx 0x{tx_id}{_}`")
