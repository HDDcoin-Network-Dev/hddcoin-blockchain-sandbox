# -*- coding: utf-8 -*-
# NOTES:
#  - this file contains the CLI implementation for `chia hodl programs`
...
from chia.hodl import BYTES_PER_HDD
...
async def cmd_programs(hodlRpcClient: HodlRpcClient,
    ...
    if len(programs) == 0:
        ...
        print(f"{C}The following Chia HODL programs are currently available:{_}")
        print(f"{W}{'-' * 83}{_}")
        ...
        print(f"{W}{'-' * 83}{_}")
        print(f"{C}To commit funds to one of these programs, please use `{Y}chia hodl commit{C}`.{_}")
