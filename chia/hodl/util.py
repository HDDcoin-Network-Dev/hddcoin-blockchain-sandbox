...
import chia.hodl
import chia.util
import chia.util.bech32m as bech32m
from chia.consensus.coinbase import create_puzzlehash_for_pk
from chia.consensus.default_constants import DEFAULT_CONSTANTS
from chia.hodl.ContractDetails import ContractDetails
from chia.rpc.full_node_rpc_client import FullNodeRpcClient
from chia.rpc.wallet_rpc_client import WalletRpcClient
from chia.types.blockchain_format.sized_bytes import bytes32, bytes8
...
def loadConfig() -> th.Dict[str, th.Any]:
    vlog(1, f"Loading config file from: {chia.util.default_root.DEFAULT_ROOT_PATH}")
    config = chia.util.config.load_config(chia.util.default_root.DEFAULT_ROOT_PATH,
                                             "config.yaml")
    return config
...
async def getFullNodeRpcClient(config: th.Dict[str, th.Any],
    ...
    rpc_port = chia.util.ints.uint16(rpc_port)
    client = await FullNodeRpcClient.create(config["self_hostname"],
                                            rpc_port,
                                            chia.util.default_root.DEFAULT_ROOT_PATH,
                                            config)
    return client
...
def getOnlyFingerprint(config: th.Dict[str, th.Any],
    ...
    keychain = chia.util.keychain.Keychain()
...
async def getWalletRpcClient(config: th.Dict[str, th.Any],
    ...
    if not loginRet["success"]:
        ...
        keyCount = len(chia.util.keychain.Keychain().get_all_private_keys())
...
async def _walletIdExists(walletRpcClient: chia.rpc.wallet_rpc_client.WalletRpcClient,
                          wallet_id: int,
                          ) -> bool:
...
async def verifyWalletFunds(walletRpcClient: chia.rpc.wallet_rpc_client.WalletRpcClient,
                            wallet_id: int,
    ...
    spendable_hdd = decimal.Decimal(spendable_bytes) / chia.hodl.BYTES_PER_HDD
    max_send_hdd = decimal.Decimal(max_send_bytes) / chia.hodl.BYTES_PER_HDD
    ...
    if recentCoinSpent == True:
        if numRecords == 1:
            ...
            raise exc.HodlError(f"HODL contract exists, but has either been cancelled or paid out. "
                                f"Check status with `chia hodl show -C {contract_id}`.")
        else:  # Wat?
            raise exc.HodlError(f"Impossible cancel condition encountered with contract "
                                f"{contract_id}. PLEASE NOTIFY THE CHIA TEAM!")
...
def getPkSkFromFingerprint(fingerprint: int,
    ...
    allPrivateKeyInfo = chia.util.keychain.Keychain().get_all_private_keys()
...
async def callCliCmdHandler(handler: th.Callable,
    ...
    if fingerprint is None:
        ...
        fingerprints = [pki[0].get_g1().get_fingerprint()
                        for pki in chia.util.keychain.Keychain().get_all_private_keys()]
        ...
        if numFingerprints == 0:
            ...
            if not response or response[0].lower() == "q":
                print(f"Aborted.")
                print(f"To see wallet balances for these keys, use `{Y}chia wallet show{_}`.")
                return
            try:
    ...
    #  - every `chia hodl` operation requires a call to the HODL server
    ...
    try:
        ...
        if keyCount:
            print(f"{R}ERROR: {Y}Unknown fingerprint. Please check `{W}chia keys show{Y}`{_}")
        else:
            print(f"{R}ERROR: {Y}No keys found! HODL'ing requires a fully synced wallet.{_}")
        return
    except Exception as e:
    ...
    if fullNodeRpcPort is not None:
        ...
        fullNodeRpcClient = await chia.hodl.util.getFullNodeRpcClient(config,
                                                                         fullNodeRpcPort)
    ...
    if walletRpcPort is not None:
        ...
        sys.stdout.flush()
        try:
            walletRpcClient = await chia.hodl.util.getWalletRpcClient(config,
                                                                         fingerprint,
                                                                         walletRpcPort)
        except Exception as e:
            print(f"{R}ERROR{_}")
    ...
    try:
        ...
        print(f"{G}  --> {C}Please report this situation to the Chia team! {W}This is unexpected.{_}")
        ...
        if "HTTP STATUS 502" in e.msg:
            print(f"{R}Connection error: {Y}The HODL server is not available right now.\n"
                  ...
                  f"If this problem persists, please contact the Chia team!{_}")
        ...
        msg = (f"{R}An unexpected error happened: {Y}{unhandledExc!r}{_}\n"
               f"{G} --> {C}Please report this to the Chia team (unexpected errors are bad!){_}")
        print(msg)
