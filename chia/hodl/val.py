...
from chia.hodl import exc as exc
from chia.hodl.ContractDetails import ContractDetails
from chia.hodl.util import vlog, puzhash2addr
...
def _validatePuzzleHash(addr: str, reveal: str) -> bytes32:
    ...
    ph = chia.hodl.util.addr2puzhash(addr)
...
def validateContract(# Given to server...
    ...
    #
    #    1. The "client" --> This chia application (i.e. this code) or the person running it
    ...
    #
    # Although the Chia team are certainly a trustable bunch and can be expected to provide the
    ...
    #    B) a secure way is provided for the server (i.e. the Chia team) to add the guaranteed
    ...
    #         - the Chia team gets reward funds from a HODL reserve in the pre-farm funds
    ...
    #        the guaranteed reward is returned to the Chia HODL reserve
    ...
    ex_payout_ph = f"0x{chia.hodl.util.addr2puzhash(ex_payout_address)}"
